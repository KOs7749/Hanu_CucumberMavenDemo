...
import dinhthe.helpers.SystemsHelpers;
import dinhthe.reports.AllureManager;
...
public class WebUI {
    ...
    @Step("Check element not existing: {0}")
    public static Boolean checkElementNotExist(By by) {
        WebUI.waitForPageLoaded();
        List<WebElement> listElement = WebUI.getWebElements(by);

        if (listElement.isEmpty()) {
            LogUtils.info("Element does not exist: " + true + " --- " + by);
            return true;
        } else {
            LogUtils.info("Element exists: " + false + " --- " + by);
            return false;
        }
    }
